...
A **tutorial** C++ example that implements **as much of** the BACnet **B-AACC
(Advanced Access Control Controller)** profile as the standard CAS BACnet Stack's
...
Schedule/Calendar, a File object, and Backup/Restore. What B-AACC requires but
...
tools/build-stack-static.sh BACnetProfileExample-B-AACC-CPP   # from the series root
...
```bash
./build/BACnetExampleBAACC [--port 47808] [--deviceID 389010]   # Linux/macOS
.\build\Release\BACnetExampleBAACC.exe [--port 47808] [--deviceID 389010]   # Windows
```
...
- Implement the B-AACC services the stack supports; expose **every required
  property** of each object for Protocol_Revision 24. Anything B-AACC requires
