...
// =============================================================================
// BACnet Profile Example - B-AACC (Advanced Access Control Controller) - C++
//
// This example implements as much of the BACnet "B-AACC" (Advanced Access
// Control Controller) device profile as the CAS BACnet Stack's CUSTOMER-FACING API
// supports today. It is seeded from BACnetProfileExample-B-ACC-CPP (Access Control
// Controller, Wave 3, released v1.0.0) and extends it with an Access User object
// plus the A-side (client) SendReadProperty/SendWriteProperty/SendSubscribeCOV keys
// - see section 2.5 below. B-ACC-CPP itself was seeded from B-LSC (Life Safety
// Controller) WITH THE LIFE-SAFETY OBJECTS/CALLBACKS REMOVED ENTIRELY (not left as
// dead code - see the "removed, not disabled" note below) and the access family
// added in their place; that lineage is why several comments below still reference
// B-LSC's own file header as the original source of a pattern.
//
// A B-AACC (ANSI/ASHRAE 135, Annex L.6) must support everything a B-ACC does, PLUS the
// A-side (initiating) services below - the delta this file adds on top of B-ACC-CPP:
//
...
//     DS-ACAD-A             - Access Control Access Doors, initiate (the peer's Access
//                              Door object is the A-side's actual read/write/COV target),
//     DS-ACCDI-A             - Access Control Credential Data Input, initiate - NOT
//                              implemented; see TODO.md #7 (re-confirmed against this
//                              repo's own pin, not assumed stale from the profile card),
...
//     Access User 1                "Denim"      (User_Type; WRITABLE Global_Identifier - profile
//                                                 delta object, Credentials left unserved: same
//                                                 constructed-type gap class as #2046, see TODO.md)
...
#include <stdio.h>
#include <string.h>
#include <stdlib.h> // atoi() - used by ParsePeerArgs()
...
// with --deviceID.
static uint32_t g_deviceInstance = 389010;
...
static const char* DEVICE_DESCRIPTION =
    "Chipkin CAS BACnet Stack example - B-AACC (Advanced Access Control Controller) profile. "
...
static const char* MODEL_NAME = "CAS BACnet Stack Example - B-AACC";

// DeviceCommunicationControl / ReinitializeDevice password. "" = no password
// required. Pick a real password (and store it somewhere other than a compiled-in
// string literal) before shipping a product - this is what stops a stranger on
// the network from disabling your device's communications or forcing a restart.
...
// Access User 1 ("Denim")'s User_Type (318, BACnetAccessUserType - REQUIRED, but
// the stack's own "Generic Enumerated default: 0" (property-profile-reference.md)
// already answers it correctly with no callback needed) and Credentials (265,
// BACnetLIST of BACnetDeviceObjectReference - REQUIRED, but grepped against
// CASBACnetStackDLL.h at the pin: no customer-facing Get callback exists for a
// BACnetLIST of BACnetDeviceObjectReference, same class of gap as TODO.md #2046
// / issue #2046 for Access Credential/Rights' constructed-type properties - left
// unserved here for the same reason, not a new defect).
static const uint32_t PROPERTY_IDENTIFIER_USER_TYPE = 318;
static const uint32_t PROPERTY_IDENTIFIER_CREDENTIALS = 265;
...
static uint32_t g_userGlobalIdentifier = 0; // Access User 1 ("Denim")
...
bool GetPropertyUnsignedInteger(const uint32_t deviceInstance, const uint16_t objectType,
    ...
    // Coral's writable Global_Identifier, Cyan's writable Global_Identifier, and
    // Denim's (Access User 1) writable Global_Identifier.
    if ((objectType == OBJECT_TYPE_ACCESS_CREDENTIAL || objectType == OBJECT_TYPE_ACCESS_RIGHTS ||
         objectType == OBJECT_TYPE_ACCESS_ZONE || objectType == OBJECT_TYPE_ACCESS_USER) &&
        ...
        *value = (objectType == OBJECT_TYPE_ACCESS_CREDENTIAL) ? g_credentialGlobalIdentifier :
                 (objectType == OBJECT_TYPE_ACCESS_RIGHTS) ? g_rightsGlobalIdentifier :
                 (objectType == OBJECT_TYPE_ACCESS_ZONE) ? g_zoneGlobalIdentifier : g_userGlobalIdentifier;
        return true;
    }
...
bool GetPropertyCharString(const uint32_t deviceInstance, const uint16_t objectType,
    ...
    if (propertyIdentifier == PROPERTY_IDENTIFIER_OBJECT_NAME) {
        ...
        if (objectType == OBJECT_TYPE_ACCESS_USER && objectInstance == 1) {
            return ReturnCharacterString("Denim", value, valueElementCount, maxElementCount, encodingType);
        }
...
bool SetPropertyUnsignedInteger(const uint32_t deviceInstance, const uint16_t objectType,
    ...
    if (objectType == OBJECT_TYPE_ACCESS_USER && objectInstance == 1) {
        g_userGlobalIdentifier = value;
        printf("WriteProperty: Access User 1 (Denim) Global_Identifier <- %u\n", value);
        return true;
    }
    return false;
}
...
// -----------------------------------------------------------------------------
// 2.5. A-side (client) keys - SendReadProperty / SendWriteProperty /
//      SendSubscribeCOV to a peer device. This is the profile-card delta on
//      top of everything B-ACC already serves: B-AACC additionally INITIATES
...
// Peer connection defaults. Overridable with --peerIp / --peerPort so the peer
// can be run on another host/port; defaults assume a same-host peer instance
// started with `--port 47809` (this device's own default --port is 47808).
static uint8_t g_peerIp[4] = { 127, 0, 0, 1 };
static uint16_t g_peerPort = 47809;
// Access Door 1 ("Cobalt") on the peer - the object this file's A-side keys
// exercise. Same object type/instance/property as this file's own Cobalt-
// pattern objects use elsewhere in this series (see docs/colour-table.md).
static const uint32_t PEER_ACCESS_DOOR_INSTANCE = 1;
static const uint8_t BACNET_DATATYPE_ENUMERATED = 9;
static const uint32_t PEER_DOOR_VALUE_LOCK = 0;
static const uint32_t PEER_DOOR_VALUE_UNLOCK = 1;
static uint32_t g_peerDoorDemoValue = PEER_DOOR_VALUE_UNLOCK; // toggles each SendWriteProperty demo

// Minimal manual parse for --peerIp/--peerPort: NOT in common/ (common/ is
// vendored verbatim from B-SS-CPP per the series' Global constraints, so
// app-specific argument parsing that only this file needs lives here instead
// of being added to the shared helper).
static void ParsePeerArgs(int argc, char** argv) {
    for (int i = 1; i < argc - 1; ++i) {
        if (strcmp(argv[i], "--peerPort") == 0) {
            int p = atoi(argv[i + 1]);
            if (p > 0 && p <= 65535) {
                g_peerPort = (uint16_t)p;
            } else {
                printf("Warning: ignoring invalid --peerPort \"%s\"; using %u.\n", argv[i + 1], g_peerPort);
            }
        } else if (strcmp(argv[i], "--peerIp") == 0) {
            unsigned int a = 0, b = 0, c = 0, d = 0;
            if (sscanf(argv[i + 1], "%u.%u.%u.%u", &a, &b, &c, &d) == 4 &&
                a <= 255 && b <= 255 && c <= 255 && d <= 255) {
                g_peerIp[0] = (uint8_t)a; g_peerIp[1] = (uint8_t)b;
                g_peerIp[2] = (uint8_t)c; g_peerIp[3] = (uint8_t)d;
            } else {
                printf("Warning: ignoring invalid --peerIp \"%s\".\n", argv[i + 1]);
            }
        }
    }
}

static void PeerConnString(uint8_t out[6]) {
    out[0] = g_peerIp[0]; out[1] = g_peerIp[1]; out[2] = g_peerIp[2]; out[3] = g_peerIp[3];
    out[4] = (uint8_t)(g_peerPort >> 8);
    out[5] = (uint8_t)(g_peerPort & 0xFF);
}

// Pack the 15-byte ReadProperty element BACnetStack_SendReadProperty/
// SendWriteProperty expect (same layout used across this series - see
// B-OD-CPP's PackReadPropertyElement for the worked byte-by-byte diagram).
static void PackReadPropertyElement(uint8_t out[15], uint16_t objectType,
                                    uint32_t objectInstance, uint32_t propertyIdentifier) {
    out[0] = 0;
    out[1] = (uint8_t)(objectType & 0xFF);
    out[2] = (uint8_t)((objectType >> 8) & 0xFF);
    out[3] = (uint8_t)(objectInstance & 0xFF);
    out[4] = (uint8_t)((objectInstance >> 8) & 0xFF);
    out[5] = (uint8_t)((objectInstance >> 16) & 0xFF);
    out[6] = (uint8_t)((objectInstance >> 24) & 0xFF);
    out[7] = (uint8_t)(propertyIdentifier & 0xFF);
    out[8] = (uint8_t)((propertyIdentifier >> 8) & 0xFF);
    out[9] = (uint8_t)((propertyIdentifier >> 16) & 0xFF);
    out[10] = (uint8_t)((propertyIdentifier >> 24) & 0xFF);
    out[11] = 0; out[12] = 0; out[13] = 0; out[14] = 0;
}

// DS-RP-A: read the peer's Access Door 1 (Cobalt) Present_Value.
static void SendReadPropertyToPeer() {
    uint8_t conn[6];
    PeerConnString(conn);
    uint8_t element[15];
    PackReadPropertyElement(element, OBJECT_TYPE_ACCESS_DOOR, PEER_ACCESS_DOOR_INSTANCE,
                            PROPERTY_IDENTIFIER_PRESENT_VALUE);
    uint8_t invokeId = 0;
    if (BACnetStack_SendReadProperty(&invokeId, READ_PROPERTY_SERVICE_TYPE, conn, sizeof(conn),
                                     NETWORK_PORT_INSTANCE, 0, NULL, 0, 1, element, sizeof(element))) {
        printf("A-side: sent SendReadProperty to peer %u.%u.%u.%u:%u (Access Door %u Present_Value).\n",
               g_peerIp[0], g_peerIp[1], g_peerIp[2], g_peerIp[3], g_peerPort, PEER_ACCESS_DOOR_INSTANCE);
    } else {
        printf("A-side: FYI failed to send SendReadProperty to peer.\n");
    }
}

// DS-WP-A: write (toggle lock/unlock) the peer's Access Door 1 (Cobalt).
static void SendWritePropertyToPeer() {
    uint8_t conn[6];
    PeerConnString(conn);
    char valueString[8];
    snprintf(valueString, sizeof(valueString), "%u", g_peerDoorDemoValue);
    uint16_t valueLength = (uint16_t)strlen(valueString);
    uint8_t element[19 + 8];
    PackReadPropertyElement(element, OBJECT_TYPE_ACCESS_DOOR, PEER_ACCESS_DOOR_INSTANCE,
                            PROPERTY_IDENTIFIER_PRESENT_VALUE);
    element[0] = 0x02; // flags: bit1 = use priority
    element[15] = BACNET_DATATYPE_ENUMERATED;
    element[16] = 8; // priority
    element[17] = (uint8_t)(valueLength & 0xFF);
    element[18] = (uint8_t)((valueLength >> 8) & 0xFF);
    memcpy(element + 19, valueString, valueLength);
    uint8_t invokeId = 0;
    if (BACnetStack_SendWriteProperty(&invokeId, WRITE_PROPERTY_SERVICE_TYPE, conn, sizeof(conn),
                                      NETWORK_PORT_INSTANCE, 0, NULL, 0, 1, element,
                                      (uint16_t)(19 + valueLength))) {
        printf("A-side: sent SendWriteProperty to peer (Access Door %u Present_Value <- %s [%s]).\n",
               PEER_ACCESS_DOOR_INSTANCE, valueString,
               g_peerDoorDemoValue == PEER_DOOR_VALUE_UNLOCK ? "unlock" : "lock");
        g_peerDoorDemoValue = (g_peerDoorDemoValue == PEER_DOOR_VALUE_UNLOCK) ? PEER_DOOR_VALUE_LOCK : PEER_DOOR_VALUE_UNLOCK;
    } else {
        printf("A-side: FYI failed to send SendWriteProperty to peer.\n");
    }
}

// DS-COV-B (the initiating half - SubscribeCOV): subscribe to the peer's
// Access Door 1 (Cobalt) Present_Value so this device observes the peer's
// state changes, including the one SendWritePropertyToPeer() itself causes.
static void SendSubscribeCOVToPeer() {
    uint8_t conn[6];
    PeerConnString(conn);
    uint8_t invokeId = 0;
    if (BACnetStack_SendSubscribeCOV(&invokeId, 1 /*subscriberProcessIdentifier*/,
                                     OBJECT_TYPE_ACCESS_DOOR, PEER_ACCESS_DOOR_INSTANCE,
                                     true /*issueConfirmedNotifications*/, 300 /*lifetime seconds*/,
                                     conn, sizeof(conn), NETWORK_PORT_INSTANCE, 0, NULL, 0)) {
        printf("A-side: sent SendSubscribeCOV to peer (Access Door %u, 300s lifetime).\n",
               PEER_ACCESS_DOOR_INSTANCE);
    } else {
        printf("A-side: FYI failed to send SendSubscribeCOV to peer.\n");
    }
}

// NOTE, verified against CASBACnetStackDLL.h at the pin: there is no
// customer-facing callback to observe a received ReadProperty-Ack, a
// WriteProperty SimpleAck, or an unsolicited/subscribed COV notification
// (grepped for RegisterCallbackReceived*Ack / *COVNotification*: zero hits;
// the only related, test-tool-only hook is
// BACnetStack_RegisterCallbackAuditNotificationReceived, per
// CASBACnetStackDLL.h's own PR #193 note). So this file cannot log the
// peer's replies on its own console the way it logs its own outbound sends
// below - the three Send* calls above are confirmed to have been ACCEPTED
// BY THE LOCAL STACK for transmission (their bool return value), not that a
// reply was received. The actual request/response round trip - the ack
// bytes coming back from the peer - is verified independently on the wire
// with bacpypes3/BAC0 (and by watching the peer B-ACDC-CPP instance's own
// console for the ReadProperty/WriteProperty it answers). See README.md's
// "Verify" section for that evidence; this is not a new gap filed against
// the stack, since AE-AC-B (TODO.md #1) already establishes the same
// "generation/consumption of async BACnet traffic beyond confirmed
// service replies isn't on the customer surface" pattern for this pin.
...
int main(int argc, char** argv) {
    ...
    ParsePeerArgs(argc, argv); // --peerIp / --peerPort - the A-side target (see section 2.5)
    ...
    if (!BACnetStack_AddObject(g_deviceInstance, OBJECT_TYPE_ACCESS_USER, 1)) {
        printf("Error: Failed to add Access User 1 (Denim).\n"); return 1;
    }
    ...
    // --- Coral / Cyan / Ebony / Denim: Global_Identifier writable ------------
    if (!BACnetStack_SetPropertyWritable(g_deviceInstance, OBJECT_TYPE_ACCESS_CREDENTIAL, 1,
        ...
        !BACnetStack_SetPropertyWritable(g_deviceInstance, OBJECT_TYPE_ACCESS_ZONE, 1,
                                         PROPERTY_IDENTIFIER_GLOBAL_IDENTIFIER, true) ||
        !BACnetStack_SetPropertyWritable(g_deviceInstance, OBJECT_TYPE_ACCESS_USER, 1,
                                         PROPERTY_IDENTIFIER_GLOBAL_IDENTIFIER, true)) {
        printf("Error: Failed to make Global_Identifier writable on Coral/Cyan/Ebony/Denim.\n"); return 1;
    }
    ...
    printf("FYI: A-side peer target %u.%u.%u.%u:%u (override with --peerIp/--peerPort). "
           "Keys: 'd' SendReadProperty, 'w' SendWriteProperty, 'r' SendSubscribeCOV - all to the peer's Access Door 1.\n",
           g_peerIp[0], g_peerIp[1], g_peerIp[2], g_peerIp[3], g_peerPort);

    // Fire all three A-side keys once automatically at start-up (in addition to
    // being available on demand via 'd'/'w'/'r' below), so a scripted wire-test
    // run (e.g. this file launched from a test harness with no interactive
    // keyboard) still exercises DS-RP-A / DS-WP-A / DS-COV-B's initiating half
    // without requiring a human to press a key. The peer is expected to already
    // be running - see README.md's Verify section.
    SendReadPropertyToPeer();
    SendWritePropertyToPeer();
    SendSubscribeCOVToPeer();
    ...
    while (running) {
        ...
        switch (CASExampleHelper::PollKey()) {
            ...
            case CASExampleHelper::KeyCommand::DemoAdvance:
                ...
                break;
            case CASExampleHelper::KeyCommand::DiscoverRemote: // 'd' - A-side DS-RP-A
                SendReadPropertyToPeer();
                break;
            case CASExampleHelper::KeyCommand::WriteGroupDemo: // 'w' - A-side DS-WP-A
                SendWritePropertyToPeer();
                break;
            case CASExampleHelper::KeyCommand::RouterAnnounce: // 'r' - A-side DS-COV-B SendSubscribeCOV
                SendSubscribeCOVToPeer();
                break;
